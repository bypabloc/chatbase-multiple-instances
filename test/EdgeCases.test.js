...
const mockChatbaseManagerClass = class ChatbaseManager {
  ...
  validateImportData(importedData) {
    try {
      ...
      if (!isValidData) {
        alert('El archivo JSON contiene datos inválidos. Revisa la consola para más detalles.')
        return false
      }
...
describe('Edge Cases and Error Handling', () => {
  ...
  describe('DOM Edge Cases', () => {
    ...
    it('should handle malformed DOM structure', () => {
      ...
      Object.defineProperty(grid, 'appendChild', {
        value: () => { throw new Error('DOM error') },
        writable: false
      })
      ...
      expect(() => manager.renderExperts()).not.toThrow()
      expect(console.error).toHaveBeenCalledWith('Error rendering experts:', expect.any(Error))
    })
  ...
  describe('Data Validation Edge Cases', () => {
    it('should handle null/undefined bot data', () => {
      expect(() => manager.createExpertCard(null)).toThrow('Invalid bot data provided to createExpertCard')
      expect(() => manager.createExpertCard(undefined)).toThrow('Invalid bot data provided to createExpertCard')
    ...
    it('should validate import data size limits', () => {
      const emptyArray = []
      const tooManyBots = Array(101).fill().map((_, i) => ({
        id: `bot-${i}`,
        name: `Bot ${i}`,
    ...
    it('should validate individual bot data thoroughly', () => {
      const invalidBots = [
        { id: '', name: 'Valid Name', description: 'Valid', chatbaseId: 'VALID', avatar: null, isDefault: false },
        { id: 'valid', name: '', description: 'Valid', chatbaseId: 'VALID', avatar: null, isDefault: false },
        { id: 'valid', name: 'Valid', description: '', chatbaseId: 'VALID', avatar: null, isDefault: false },
        { id: 'valid', name: 'Valid', description: 'Valid', chatbaseId: '', avatar: null, isDefault: false },
        { id: 'valid', name: 'Valid', description: 'Valid', chatbaseId: 'VALID', avatar: 123, isDefault: false },
        { id: 'valid', name: 'Valid', description: 'Valid', chatbaseId: 'VALID', avatar: null, isDefault: 'yes' }
      ]

      expect(manager.validateImportData(invalidBots)).toBe(false)
      expect(alert).toHaveBeenCalledWith('El archivo JSON contiene datos inválidos. Revisa la consola para más detalles.')
    })
  })
  ...
  describe('Chat Instance Edge Cases', () => {
    ...
    it('should handle opening chat with invalid data', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Invalid chatbotId or botId provided to openChatbase')
    })
    ...
    it('should handle transition state correctly', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('Transition in progress, ignoring openChatbase call')
    ...
    it('should handle minimizing non-existent instances', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Cannot minimize: invalid botId or instance not found: non-existent')
    })

    it('should handle restoring non-existent instances', () => {
      expect(() => manager.restoreChatInstance('non-existent')).not.toThrow()
      expect(console.error).toHaveBeenCalledWith('Cannot restore: invalid botId or instance not found: non-existent')
    })
    ...
    it('should handle orphaned chat instances', () => {
      ...
      expect(console.warn).toHaveBeenCalledWith('Container for bot test-bot no longer exists, recreating...')
    })
  })
  ...
  describe('Floating Button Edge Cases', () => {
    it('should handle floating button creation with null bot', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Cannot create floating button: bot is null/undefined')
    })
    ...
    it('should handle floating button creation failure', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Error creating floating chat button:', expect.any(Error))
  ...
  describe('Viewport and Responsive Edge Cases', () => {
    it('should handle viewport detection errors', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Error checking mobile viewport:', expect.any(Error))
    })
  ...
  describe('Event Handling Edge Cases', () => {
    ...
    it('should handle closeConfig with missing modal', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Error closing config modal:', expect.any(Error))
    })
  })
  ...
  describe('Memory Management', () => {
    ...
    it('should handle cleanup errors gracefully', () => {
      ...
      expect(() => manager.cleanupAllInstances()).not.toThrow()
      expect(console.error).toHaveBeenCalledWith('Error cleaning up instances:', expect.any(Error))
    })
  })
})