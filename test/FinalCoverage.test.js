...
describe('Final Coverage Push - Target 95%', () => {
  ...
  describe('Execute Import Function - Lines 1030-1036', () => {
    it('should execute complete import flow with updateFloatingChatButton', () => {
      ...
      expect(localStorage.setItem).toHaveBeenCalledWith('chatbaseBots', JSON.stringify(validData))
  ...
  describe('Delete Bot Function - Lines 1167-1179', () => {
    ...
    it('should delete bot with confirmation and cleanup instance - lines 1167-1179', () => {
      ...
      expect(global.confirm).toHaveBeenCalledWith('¿Estás seguro de que quieres eliminar este bot?')
  ...
  describe('Additional Coverage for Edge Cases', () => {
    it('should handle import validation errors properly', () => {
      ...
      expect(global.alert).toHaveBeenCalledWith('El archivo JSON no tiene el formato correcto. Cada bot debe tener: id, name, description, chatbaseId, avatar, isDefault')
    })
    ...
    it('should handle non-array import data', () => {
      ...
      expect(isValid).toBe(false)
      expect(global.alert).toHaveBeenCalledWith('El archivo JSON debe contener un array de bots')
    })
